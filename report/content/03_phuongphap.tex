...
\begin{itemize}
    \item \textbf{Tầng Chiến lược (PPO Meta-Controller):} Agent PPO quan sát vector trạng thái $\mathbf{s}(t) = [\Gamma(t), A(t), I_{risk}(t), \dot{v}_{norm}(t), u(t-1)]$ và xuất ra 4 hành động liên tục: $[\Delta K_p, \Delta K_i, \Delta K_d, \Delta\theta_{friction}]$, điều chỉnh online các tham số PID tại mỗi bước thời gian. Chính sách (policy) được tham số hóa bởi mạng perceptron đa tầng (\textit{MlpPolicy}) và tối ưu hóa qua thuật toán PPO-Clip (Schulman et al., 2017).
